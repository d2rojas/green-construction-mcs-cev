...
const InteractiveChartViewer = ({ charts }) => {
  ...
  const loadAllChartData = async () => {
    ...
    for (const chart of charts) {
      // Skip charts without name or MCS power profile charts as they're not needed
      if (!chart.name || (chart.name.includes('mcs_') && chart.name.includes('power_profile'))) {
        continue;
      }
      
      if (chart.type === 'INTERACTIVE' && (chart.csvData || chart.csvDataUrl)) {
        try {
          let csvText;
          
          // Use direct CSV data if available, otherwise fetch from URL
          if (chart.csvData) {
            csvText = chart.csvData;
            console.log(`Using direct CSV data for ${chart.name}`);
          } else if (chart.csvDataUrl) {
            const response = await fetch(`http://localhost:3002${chart.csvDataUrl}`);
            if (response.ok) {
              csvText = await response.text();
            } else {
              console.error(`Failed to fetch CSV data for ${chart.name}`);
              continue;
            }
          }
          ...
          if (csvText) {
            ...
            if (processedData && processedData.datasets && processedData.datasets.length > 0) {
              ...
              console.log(`Successfully processed chart data for ${chart.name}`);
            } else {
              console.log(`No valid data processed for ${chart.name}`);
            }
          }
        } catch (error) {
    ...
    if (!chart.name) {
      return (
        <div className="text-center p-4">
          <p className="text-muted">Chart name not available</p>
        </div>
      );
    }
