...
$CALL GDXXRW Input_CEV.xlsx Index=ParamsAndSets!A1
$GDXIN Input_CEV.gdx
                                                     // xlsx to gdx, call for input file
...
Scalar rho_miss     /10/;                       // Penalty for missed work at construction nodes ($/kW)
...
eq24, eq25, eq26, eq27, eq28, eq29, eq30, eq31, eq32, eq33;
...
* Objective function
objfun.. COP =E= sum((m,t), lambda_CO2(t) * P_ch_tot(m,t)*DT + lambda_buy_UCSD(t) * P_ch_tot(m,t)*DT) + sum((i,e,t), P_missed_work(i,e,t)*rho_miss);     // Objective function: minimizing both carbon cost and electricity cost
...
eq20(i,e,t).. P_work(i,e,t) =l= R_work(i,e,t);     // The work should be done as scheduled, thus power consumed by CEV e in time interval t less than or equal to power for scheduled work

eq21(i,e,t).. P_missed_work(i,e,t) =e= R_work(i,e,t) - P_work(i,e,t);     // The missed work equals to scheduled work minus accomplished work

eq22(m,i,t).. sum(e, e_conmcs(m,i,e,t)) =l=  C_MCS;  // Limits the number of connected CEVs to the MCS m in the same time interval

eq23(m,i,e,t)..   e_conmcs(m,i,e,t) =l= place(i,e);  // Each CEV must be connected to MCS m in its designated construction node, "place" is a binary parameter

eq24(m,j)..  sum((i,t)$(ord(i)<>ord(j)), x(m,i,j,t)) =e= sum((i,t)$(ord(i)<>ord(j)), x(m,j,i,t));  // x determines whether MCS m goes from node i to node j, This equality ensures that number of
                                                                                                   // arrivals and departures for each node is equal

eq25(m,i,t)$(ord(t)>1).. beta_arr(m,i,t) - delta_dep(m,i,t) =e= alpha(m,i,t) - alpha(m,i,t-1);    // This equality establishes a balance between arrival/departure times and connection status of MCS m
...
eq26(m,t).. sum(i, alpha(m,i,t)) =l= 1;         // MCS can connect only one node in time interval t

eq27(m,t).. sum(i, beta_arr(m,i,t)) =l= 1;      // MCS can arrive to only one node in time interval t

eq28(m,t).. sum(i, delta_dep(m,i,t)) =l= 1;    //  MCS can depart from only one node in time interval t

eq29(m,arcs(i,j),t,tt)$(ord(tt) = ord(t) + T_arcs(i,j)).. x(m,i,j,tt) =l= delta_dep(m,i,t);   // When MCS m departs from node i at time interval t to travel to node j, it can only arrive at node j after completing the travel time
                                                                                              // Therefore, x can take a value of 1 only when tt = t + T_arcs

eq30(m,j,t)..  beta_arr(m,j,t) =e= sum(i$(ord(i) <> ord(j)), x(m,i,j,t));     // MCS m can arrive node j if x equals to 1 in the same time interval

eq31(m,i,t)..  beta_arr(m,i,t) + delta_dep(m,i,t) =l= 1;                     // MCS m cannot arrive and depart in the same time interval

eq32(m,i2(i)).. sum(t,  beta_arr(m,i,t)) =l= 1;                              // MCS m cannot make multiple visits to construction nodes during simlation

eq33(m,i2(i)).. sum(t, delta_dep(m,i,t)) =l= 1;                              // MCS m cannot make multiple visits to construction nodes during simlation
